create procedure [dbo].[SalesInvoiceCreateInternal] @xml xml, @Id TIdentifier out
as
begin
	set nocount on;
	...
	insert into [dbo].[SalesInvoice]
		([State], [FileAs], [Number], [DocumentDate], [DueDate], [OrganizationId], [AccountId],
		[BudgetId], [OperatingBudgetId], [BudgetItemGroupId], [BudgetItemId], [ResponsibleEmployeeId], [Subject], [IsCash],
		[Value], [VATRateId], [VATValue], [Comments], [Created], [CreatedBy], [Modified], [ModifiedBy])
	values
		(1, @FileAs, @Number, @DocumentDate, @DueDate, @OrganizationId, @AccountId,
		@BudgetId, @OperatingBudgetId, @BudgetItemGroupId, @BudgetItemId, @ResponsibleEmployeeId, @Subject, @IsCash, @Value, @VATRateId, @VATValue, @Comments,
