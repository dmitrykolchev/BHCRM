create procedure [dbo].[InventoryStatementBrowse] @filter xml
as
begin
	set nocount on;
	...
	select
		...
		[RowVersion]
	from
		[dbo].[InventoryStatement]
	where
		(@Id is null or [Id] = @Id)
		and
		([InventoryStatement].[DocumentDate] between @PeriodStart and @PeriodEnd)
		and
		(@OrganizationId is null or [InventoryStatement].[OrganizationId] = @OrganizationId)
		and
		(@StoragePlaceId is null or [InventoryStatement].[StoragePlaceId] = @StoragePlaceId)
		and
