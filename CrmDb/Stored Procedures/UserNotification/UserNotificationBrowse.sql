create procedure [dbo].[UserNotificationBrowse] @filter xml
as
begin
	set nocount on;

	declare @AllStates bit, @Id TIdentifier, @Presentation varchar(256), @LastId TIdentifier, @UserId TIdentifier;

	select
		@AllStates = T.c.value('AllStates[1]', 'bit'),
		@Id = T.c.value('Id[1]', 'TIdentifier'),
		@LastId = T.c.value('LastId[1]', 'TIdentifier'),
		@Presentation = T.c.value('Presentation[1]', 'varchar(256)')
	from
		@filter.nodes('/Filter') as T(c);

	set @UserId = [dbo].[GetCurrentUserId]();

	select
		[Id],
		[State],
		[FileAs],
		[DocumentTypeId],
		[DocumentId],
		[UserId],
		[Comments],
		[Created],
		[CreatedBy],
		[Modified],
		[ModifiedBy],
		[RowVersion]
	from
		[dbo].[UserNotification]
	where
		(@LastId is null or [Id] > @LastId)
	and
		[UserId] = @UserId
	and
		(@Id is null or [Id] = @Id)
	and
		(@AllStates = 1 or [State] in (select T.c.value('.[1]', 'tinyint') from @filter.nodes('Filter/State') as T(c)));

	return 0;
end
